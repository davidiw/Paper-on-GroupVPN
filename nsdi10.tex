...
by traditional VPNs.  Additionally, they provide relays when NAT traversal is
unsuccessful, but the scalability in current approaches has not been validated.
...
In the paper, we describe the components of our model as well as present
evaluate our reference implementation quantitatively to compare system and
networking overheads of the different VPN technologies focusing on latency,
bandwidth, and memory.  We also discuss our usage of the system over the past
3 years.
%\end{sloppypar}
...
VPNs include secure access to enterprise network resources from remote/unsecure
...
A brief overview of our solutions to the above problems follows and will
...
membership, we use an automated certificate authority based upon the use of
user groups.  In the case of full tunneling, P2PVPNs introduce significantly
more complexity since a simple routing table swap as done in central VPNs no
longer work. Thus, we investigate three different mechanisms for tunneling all
...
\item use of a private P2P VPN system bootstrapped as a subset of the P2P system
...
difficult.  The tradeoffs come in terms of usability.  While the use of uniquely
...
typical home network, all traffic for the VPN server is sent via
the local network's router to the VPN gateway via the Internet.  All other
...
client and server can authenticate with each other.  For a server to authenicate
...
and maintain VPN systems, in otherwords, they lack self-organization.  This
...
ambiguous monicker ``P2P VPN''.  In reality, these systems would be best
...
inpenetrable NATs, when this happens the central server either acts as a relay
...
requires knowledge of an existing peer in the system.  It uses this endpoint to
...
\begin{tabular}[c]{|m{1.5cm}||m{2.1cm}|m{2.4cm}|m{2.1cm}|m{2.9cm}|m{3.2cm}|} \hline
...
tinc & Centralized & CA Certificates & Global knowledge &
Relay through mesh & Open Source\\ \hline

CloudVPN & Centralized & CA Certificates & Global knowledge &
Relay through mesh & Open Source\\ \hline
...
GBridge & Centralized P2P & Password at central server & Stored locally &
...
N2N & Unstructured P2P & Shared secret & Broadcast look up & NAT traversal,
decentralized relay & Open Source \\ \hline

P2PVPN & Unstructured P2P & Shared secret & Everyone has global knowledge &
...
should be easily availble for verification and the ability to ban and remove
...
ofa  binary blob generated by the server allows the user to keep his private key
...
transmitted via the overlay, but it is signficantly more difficult to prevent
...
in order to completely interrupt the broadcast, though this is feasable in our
...
With a gateway entact, the VPN software can now announce that it provides
...
public address to our routing table prior to the first outgoign communication
