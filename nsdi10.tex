...
is more secure than existing implementations.  To verify our model,
we evaluate reference implementation quantitatively to compare system and networking
...
(LAN) spanning over a public wide area network (WAN) infrastructure, creating
...
individual resources, where all resources connected to the virtual network are
configured with P2P VPN software.  Our work is significantly different
...
traffic to enterprise networked resources and encrypted Internet traffic from mobile
users at untrusted locations.  Another example in academic environments is
collaborative environments linking individuals belonging to a virtual organization
...
external provider; furthermore, they lack the support for full VPN tunneling
of traffic to Internet hosts.
...
single solution that deals with the following in an integrated manner: 
...
an existing public P2P system to create a trusted subset of a P2P ring that spans only the members of a VPN group.
This enables the use of P2P routing, multicast and DHT storage/look up within a private subset of the main overlay.
...
provide mechanisms that allow two peers to establish two-hop relay links based upon
...
Section~\ref{structured_p2p} provides background on P2P systems and previous work on IP-over-P2P overlay.
Section~\ref{p2pvpn} describes the techniques used to create structured P2P VPNs.
In Section~\ref{evaluation}, we discuss our implementation and present evaluation
...
related works, and Section~\ref{conclusions} presents concluding remarks.
...
are many kinds as summarized in Table~\ref{tab:vpn_types}.  A brief survey of popular
VPNs comprising different configurations is summarized in Table~\ref{tab:vpns}.
This section overviews core approaches in VPN designs, beginning with client configuration of VPNs, followed
by a description of different VPN server configurations as highlighted in
...
device driver, which provide the mechanisms to inject incoming packets and retrieve outgoing
...
a VPN connection that only handles \emph{internal VPN traffic only and
...
traffic} through a VPN router.  This enables a user to ensure that all their Internet
communication originates from a secure and trusted location and provides network-layer
privacy and authentication when a user is in an insecure 
environment, such as an open wireless network at coffee shop.  Both models are illustrated
...
these projects are closed, which prevents users from hosting their own
...
of at least one participant in the system in order to connect to the system. 
...
Overlay shortcuts are key to efficient routing in ring-structured P2P systems.  There are many
approaches to select shortcuts, each with different
trade-offs.  A few of these include:  maintaining large tables without
...
from a random distribution in the node ID space~\cite{symphony}.
...
and self-optimizing shortcuts to support single-hop connectivity between peers when virtual IP traffic is detected
between endpoints~\cite{wow}.

On top of the Brunet P2P library, we have implemented a VN with the
...
information exchanged in a P2P overlay. This is primarily because malicious users can
...
The DHT method acts as a event notification.  Users place their
...
bootstrapping a private overlay from an existing public overlay.  Communication within the private
overlay is completely encrypted and authenticated with only members of the
VPN allowed access.  This approach is similar to previous work suggesting the use of
a public overlay to bootstrap application-specific overlays
in~\cite{one_ring}.

In this approach, members of the VPN are the only members of the private overlay, providing a
...
of members active in the private overlay.  The joining VPN software will
...
support NAT traversal --- this model allows reusing Brunet's underlying NAT
...
discovery.  Furthermore, VPN peers can elect to use routing paths over the private overlay
for VPN traffic, and choose relays within the private overlay only. The public overlay can be used for relaying, when
...
potentially unencrypted.  Another issue that arises is that
malicious users can send spoofed packets
...
This approach recognizes that when using UDP, an application can determine
the source ports from which all application traffic would originate.
Again, this approach requires a routing rule directing all Internet traffic to
the VPN gateway though with no additional rules.  This results in
all packets being directly sent to the VN device.  Meaning that not only will
general Internet traffic process through the VPN state machine but so will P2P traffic.
...
direct it to the LAN's gateway.  This requires building of an Ethernet frame that will
be written to the physical Ethernet device.  The first step is to change the
source IP address to match the physical Ethernet's IP address, so that the gateway
will route the packet properly.  The next step is to embed the resulting packet
into an Ethernet frame prior.
The source address for the Ethernet frame can be generated using the method described
in~\cite{sc09}, and its destination address should be the LAN's gateway.
The next step is to write the packet to the physical Ethernet device, but Windows lacks
support for this operation.
Our platform independent solution uses a second TAP device bridged to the
physical Ethernet device, allowing Ethernet frames to be written indirectly to the physical
Ethernet device via the TAP device.  Incoming P2P packets from the Internet are
delivered directly to the VPN application with no ill effect even though the incoming
destination and outgoing source IP addresses do not match.  All other packets enter the system same
as before.  This method has been verified to work on both Linux and Windows using OS dependent TAP devices
...
orders of magnitude larger than passing through the VPN's packet processor.  Adding a
direct Ethernet writer in the P2P system is not a clean abstraction as a VPN portion of the
system now becomes library dependent and reduces VPN software portability.

\subsection{Autonomic Relays}
When NAT traversal using STUN~\cite{stun} fails or there are other connectivity
issues some of P2P VPNs~\cite{hamachi, gbridge} support relaying, similar
...
work~\cite{hpdc08_0,epost}.  In this work, we describe a
mechanism using triangular routing that allows peers next to each other in the
node ID space to communicate despite being unable to communicate directly
because of firewall, NAT, or Internet
disconnectivity issues.

The process for forming local relays or ''tunnels''~\cite{hpdc08_0} begins by two nodes discovering
each other via existing peers and realizing they need to be connected.
If they are unable to directly connect, they exchange
...
already have an overlapping set.  This involves having the peers connect to a member
of each other's neighbor set to proactively create an overlap, as represented in Figure~\ref{fig:relay}.
...
In a live system, we verify the accuracy and usefulness of our latency-based relay selection
algorithm in in Section~\ref{relay_eval}.
...
experiment was performed in Windows Vista 64-bit.  The platform was two virtual
machine located on the same host with a firewall preventing them from
...
Archer~\cite{archer}.  These machines at Universities and thus have a high bandwidth
...
The two apparent take aways are 1) the benefit of being able to deploy you own
