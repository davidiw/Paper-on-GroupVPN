...
This enables the use of P2P routing, multicast and DHT storage/lookup within a private subset of the main overlay.
...
system encouters two challengen in supporting full tunnels:  1) P2P traffic must not be routed
...
tradeoffs.  A few of these include:  maintaining large tables without
...
Our implementation is portable to any system that supports Tap and a C\# runtime (e.g. Mono, .Net),
...
model that is synnergistic with the group VPN interface described earlier.  This
...
processing packets coming from the VPN.  Rather than rejecting pakets
...
service we run on PlanetLab~\cite{planetlab}. PlanetLab serves as a
...
would work.  Our discovery model for maping node ID to IP works through the use of a DHT.
