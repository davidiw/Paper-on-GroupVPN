...
of a central server as a public key infrastructure (PKI) via a partially
automated web interface.  Additionally, we provide a design and implementation
of the first P2P VPN to support full tunneling, whereby all non-P2P based
...
membership, we use an automated public key infrastructure (PKI) using an intuitive
...
\setlength{\itemsep}{0pt}
\setlength{\parskip}{0pt}
\item automated group-based PKI
...
certificates and clients can verify each other by using the CA's public
certificate.  Setting up, deploying, and then maintaining security can easily
become a non-negligible task.  Most VPN systems that use a PKI
require the use of command-line utilities and setting up your own methods of
...
present partially automated PKI reliant on a self-hosting group based web
interface.  Although this does not preclude other methods of CA interaction,
we believe it provides a model that will be satisfactory for many uses cases.

The key to the group infrastructure is that
...
should be easily available for verification and the ability to police
...
CA signing certificates as he verifies new members and
polices current members.  Signed certificates can be used in decentralized systems as they
do not require communication with the CA, users can verify
that they are members of the group by verifying the CA's signature on the exchanged
