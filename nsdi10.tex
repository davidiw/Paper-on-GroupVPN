...
Internet traffic routes through a trusted third party.  To verify our model,
...
\setlength{\parskip}{0pt}
\item no dependence on centralized server(s) for creation, maintenance and tear-down of VPN links
\item full tunneling of Internet traffic
\item decentralized relay selection, where any peer can potentially be a relay
\item user-friendly, intuitive membership management in a VPN
%\item support for large and active VPN pools
\end{itemize}
%}
...
Configuring a machine as a gateway can be done with NAT software. For example, in Linux
...
This approach requires a routing rule directing all traffic to
the VPN gateway, but there are no additional routing rules.  This results in
all packets being directly sent to the VN device, leaving the VPN client software to handle
...
client's networking stack, the client does not recognize the packet because the
...
This triggers the OS to either ignore the packet, in case of UDP, or send a TCP reset message.
Thus the solution needs some form of NAT to handle receiving incoming
...
% this in unlear - approach 1 has eavesdropping, approach 2 also has a problem - so reviewers will ask, do we have a working solution, overall?
