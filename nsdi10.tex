...
system encouters two challenges in supporting full tunnels:  1) P2P traffic must not be routed
...
Ethernet device via the TAP device.  Interestingly, P2P packets that come back from the Internet are delivered
directly to the VPN application with no ill effect even though the incoming destination and outgoing source IP addresses
do not match.  Tunnel packets wrapped in P2P packets enter the VPN client and
using the existing method of delivering the packet to the application.
This method has been verified to work on both Linux and Windows using OS dependent TAP devices
and bridge utilities.
This solution does have a minor drawback, which is that an outgoing packet will
traverse the VPN stack twice, once as an application packet and the second as
a P2P packet.  This cost is probably negligible as most of the round trip time on the
packet will occur over the wide area which will be orders of magnitude slower than
the VPNs packet processor.  This solution does have a significant upside beyond
even P2P use: it fixes a security hole in existing
full tunnel models, because there is no chance of insecure traffic ever being transmitted
from the physical Ethernet device when a machine is in full tunnel mode.
...
We also present a comparison of our methods for selecting relays, age and latency,
...
discovery of physically close TURN-like servers.  Skype~\cite{skype_overview}
