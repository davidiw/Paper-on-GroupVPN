...
P. Oscar Boykin*, Renato Figueiredo*}\\
       \affaddr{*University of Florida, \#Clemson University}\\
}
...
represents the typical features of a centralized VPn and then some.  The
...
address that is routable in the virtual network address space, other changes may
be instituted by the VPN client software depending on the services provided.
...
that went with the ambiguous monicker ``P2P VPN''.  The P2P provided by these
\cite{wippien, gbridge} types of systems is direct connectivity between peers
bypassing a central server.  While direct connection is desirable, it does not
...
eavesdrop.  Finally, there has been no work on full-tunnel P2P VPNs in any of
the P2P VPN fields.
...
search time in O(Log N) to O(Log2 N) time unlike unstructured systems that
most either know all the state in the system or make random walks
\cite{unstructured}.  Some examples of structured systems can be found
...
purely decentralized mechanism.  While the work is quite usefu, we have also
...
\item a sybil-resistant free P2P system
\end{itemize}
}

A sybil-resistant system is made available through the use of the P2P
group architecture, whereby malicious peers may be able to misbehave for a
short period of time.  They wil be quickly detected, and through the use of
certificate revocation, removed from the overlay.
