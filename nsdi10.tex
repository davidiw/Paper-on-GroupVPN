...
disconnectivity issues.  To support this behavior, two nodes would discovery
each other by indirect communication via the overlay.  This would trigger a
best effort to exchange peer lists via the current set of near neighbors.  In
most cases, the peers would have at least one overlapping neighbor and the
messages would be exchanged.  From thereafter, the peers would use the
...
made it possible to exchange arbitrary information to assist in making decisions.
...
untrusted overlay system.  Decentralized security provides only probabilistic
...
The next step is to run a week long test on Planet-Lab that ensures reliability,
...
viability of our platform running on top of other structured P2P systems, namely
