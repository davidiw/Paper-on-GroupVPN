...
supporting NAT traversal using relays when necessary.  Other approaches seek to remove all
...
is more secure than existing full tunnel implementations.  To verify our model,
...
dhcp1}.  Setting the IP address of the VN device causes the system to add a new
...
centralized VPNs. Because it provides a representation of features found in most centralized VPNs, we use it as a basis for comparison with our approach.
...
The cost of passing all packets through the VPN application should be negligible since
...
FreePastry does not have the ability to form shortcuts based upon communication demands, so if two peers were
