...
we evaluate reference implementation by comparing it quantitatively 
...
users at untrusted locations.  Another example in collaborative academic environments
...
members of a VPN group providing secure P2P routing and DHT storage.
...
are many typess as summarized in Table~\ref{tab:vpn_types}.  A brief survey of popular
...
wireless network at a coffee shop, by securely proxying all Internet traffic
...
Central VPNs server operate at well-known endpoints 
...
client and server both authenticate with each other in using some some formchallenge
response protocol.

All inter-client packets flow through the central server.  In the default
configuration of OpenVPN, the clients encrypt the packet sending it to the server.
...
The current approach lacks a distributed load balancer.
...
Overlay shortcuts are key to efficient routing in ring-structured P2P systems.  
...
The DHT method acts as a event notification.  Upon a revocation,
...
1) the destination Ethernet address is the LAN gateway, 2) the
...
encrypted, if the server also hosts a website, that traffic will be 
potentially unencrypted.  Another issue that arises in the P2P case is that
...
removing any potential for eavesdropping.  In this soluiton all Internet packets with no
...
have an overlap set.  This involves having the peers connect to a each other's
...
Additionaly, we added the feature to exchange arbitrary information
...
relay selection algorithm in in Section~\ref{relay_eval}.
...
\caption{Results of the evaluation comparing latency and bandwdith of Hamachi 
...
Gate Priv. is the gateways VPN address, Gate Pub. is the gateways public
...
our different deployments of our P2P VPN, then we present our experiences
...
challenging environment environment that stresses our system and helps us assess its reliability, stability, and
...
that key, the owner needs to query for it's value, which would be the node ID of the
...
We present a novel method for client full tunneling that works in P2P and provides a level
of security not present in prior works by ensuring that only secure data is transmitted over the LAN.
The PtP and EtE links are secured via an group system
