...
A centralized VPN server can be a single point of failure, result in
...
run-time connectivity and public-key based management of VPN end-point
...
Section III introduces P2P structures and our previous work IPOP (IP over P2P).
Section IV describes the contributions of this paper, namely a feature-full
P2P VPN.  In Section V, we discuss our implementation and present evaluation
comparing centralized, P2P, and our VPN.  Finally, we give some concluding
...
Decentralized Servers and Clients & Nothing distinguishes a client from a server,
...
Once the user has connected with the overlay, the VN device
...
or VPN connection that only has passes \emph{VPN related traffic only and
...
\caption{A VPN setup in both full and split tunnel mode.  In both modes,
...
Central VPNs server operate at well-known end-points as in
...
packet
\item after positive authentication, the packet is written to the VN device
...
the server too must behave somewhat like a client by enabling a VPN end-point
via a VN device.  The VN device is configured to support NAT from the VN to
the local network and/or the Internet.  In Linux, this is achieved with minimal
...
typical Tinc and CloudVPN setup, individual users share end-points with each
...
is routed to it destined for them, depending on the message type, it will
assume the packet was meant for itm or throw the packet away.  Thus having multiple
...
their VN device.  Using the above example, the routing rule would
...
2) the lowest address in a network is reserved for VN services
...
\setlength{\itemsep}{0pt}
\setlength{\parskip}{0pt}
\item is the packet destined for some area outside of the VPN address space
\item if so, verify that full tunnel VPN is enabled
\item if so, discover a remote peer to act as a full tunnel gateway
\item after finding one, send the message to the remote peer
\end{enumerate}
