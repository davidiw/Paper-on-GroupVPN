...
\subsection*{Abstract}
%\begin{sloppypar}
Virtual Private Networks (VPNs) have traditionally used one or more managed servers.
In recent years, P2P approaches to the design of VPNs have arised, which generally fall
into two categories: 1) P2P VPN links where session management is centralized, and
2) unstructured P2P systems. Compared to traditional VPNs, these approaches can reduce management overhead 
and improve scalability by allowing nodes to connect directly, bypassing the overhead of
communicating through a VPN server.  However, existing P2P VPN approaches face manageability and 
and availability constraints: if the centralized management server is unavailable, VPN links cannot be established,
and managed proxy servers are required when NAT traversal is not possible between two peers.
In this paper we propose a novel VPN architecture that uses a structured P2P system for session management,
overlay routing, NAT traversal and proxy selection and a central server for certificate management.
Our approach provides an intuitive group-based trust model
and a simple management interface, reduces the requirements for connectivity, autonomous proxy selection
in lieu of NAT traversal. In this paper, we
describe the components of this model as well as present and evaluate our
reference implementation quantitatively to compare system and networking overheads of
the different VPN technologies focusing on latency, bandwidth, CPU, and memory.
%\end{sloppypar}

\section{Introduction}
A Virtual Private Network (VPN) provides the illusion of a Local Area Network
(LAN) spanning over a public wide area network infrastructure,
guaranteeing secure and authenticated communication amongst
participant nodes.  Common uses of VPNs include: providing access to enterprise
network resources from remote/unsecure locations, connecting distributed resources from multiple sites,
and establishing virtual LANs for multi-player video games over the
Internet.  In the context of this
paper, we focus on VPNs that provide end-to-end private links among any pair of resources, and
where each endpoint resource is configured with VPN software that makes them a peer in a structured P2P overlay.
Our work is significantly different in scope from approaches
which define VPNs `` as the 'emulation of a private Wide Area Network (WAN)
facility using IP facilities' (including the public Internet or private IP
backbones).''~\cite{ip_vpns}.  These tend to be clustered: endpoints are connected to the VPN through
one of more routers deployed at each site on a subnet unique to the
rest of the virtual network.

A centralized VPN server suffers from several drawbacks: the server is a single point of
failure, a performance bottleneck, and can compromise end-to-end security if packets need to be decapsulated and re-encapuslated at the router.
Several alternative approaches that address these issues have been conceived: 1) support for multiple VPN
...
a novel approach to forming secure, scalable, and self-configuring VPNs
through the the use of structured P2P systems. Our approach is unique in how it
uses managed server(s) as certificate authorities (CAs) to bootstrap endpoint public key credentials,
while supporting fully decentralized establishment of VPN links. Once endpoints
are configured with VPN certificates through initial interactions with a CA server,
the structured P2P virtual network overlay enables decentralized NAT traversal,
symmetric key exchange, and overlay routing, effectively enabling
all-to-all VPN communication without maintaining all-to-all connectivity with
participants.  

While VPNs that support P2P connections among endpoints do exist, they suffer from several limitations: 
session setup and NAT traversal are handled by managed servers; when they are unavailable, connections
cannot be established; proxies for nodes that are not amenable to NAT traversal, when supported, are also managed and
become performance and availability bottlenecks. Furthermore, they do not provide features such as
full-tunneling of network traffic and forwarding Internet traffic.  Unstructured P2P based
VPNs have similar issues, but have the ability to reuse the unstructured
...
lack the ability to add/revoke participants in the system in a scalable manner.
...
% what do you mean by managing participants in a live system?
...
% the above (debugging) seems out of context here
\end{itemize}
%}
...
run-time connectivity and public-key based management of VPN endpoint membership, we use an automated certificate authority based
...
central VPNs no longer work. Thus, we investigate three different mechanisms
...
peers that are physically close to each other and use them to relay
...
Overall, the main contribution made in this paper is the design and implementation of a novel structured P2PVPN overlay architecture.
Many of the individual components of our solution are not novel in themselves,
but rather the integration of these components and their interaction results in a system that is unique in supporting
%\small{
\begin{itemize}
\setlength{\itemsep}{0pt}
\setlength{\parskip}{0pt}
\item automated group-based certificate authority
\item support for full-tunneling and multiple gateways
\item decentralized NAT traversal with autonomous selection of relays for non-traversable nodes
