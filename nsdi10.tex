...
\cite{openvpn, hamachi, wippien, p2pvpn, n2n, tinc} and more, there is not a
single solution that deals with all these concerns: 
%\small {
\begin{itemize}
\setlength{\itemsep}{0pt}
\setlength{\parskip}{0pt}
\item reliance on an third party for session management
\item user-friendly, intuitive membership management in a VPN
\item full tunneling of Internet traffic
\item decentralized relay selection
\item support for large and active VPN pools
\end{itemize}
%}

In our system, we use an automated public key infrastructure (PKI), that is
freely available.  The PKI is managed through a group-based web interface
that allows manager to review applications into the group and remove malicious
users.  Furthermore, we use the bootstrapping of a private P2P system off
an existing public P2P system to avoid common trust concerns in P2P systems.
...
provide mechanisms that allow two peers to establish two-hop links based upon
stability and proximity when direct connectivity is unavailable.  The main
...
are many kinds as presented in Table~\ref{tab:vpn_types}.  A handful of popular
VPN comprising different configuration are displayed in Table~\ref{tab:vpns}.
...
blocking out all users if the shared secret becomes compromised or individual users if they act
...
difficult to set up and use.  In a system comprising of of non-experts, the
typical setup includes the use
...
tinc~\cite{tinc} & Decentralized & PKI & Global knowledge &
Relay through mesh & Open Source\\ \hline

CloudVPN~\cite{cloudvpn} & Centralized & PKI & Global knowledge &
Relay through mesh & Open Source\\ \hline
...
system and explicit specification of which links to create.  This
