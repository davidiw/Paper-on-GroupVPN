...
PVC~\cite{pvc}, support secure and direct communication amongst peers but rely
on central resources for session establishment and thus are vulnerable to
...
configuration of links between members of the virtual network, lacking the
ability to automatically reconfigure when links fail; security in these systems
does not secure communication between end hosts.  Existing P2P approaches lack
...
locations, such as homes, schools, and hotels.  Recently, a grid at La Jolla
Institute for Allergy and Immunology went live using GroupVPN and Grid Appliance
without receiving any technical support from us.  Researchers at Clemson
...
networks of over 700 nodes.

In Archer, users begin by creating an account at \url{Grid-Appliance.org} and
requesting membership in the Archer GroupVPN group.  Once access has been
granted, users can obtain configuration data that enables them to seamlessly
add resources to the grid by handing the configuration file to the Grid
Appliance initialization scripts.  Other forms of grid environments, typically
have a shared submission site, but Archer is open to all architecture
researchers and monitoring usage on a shared system would require a dedicated
administrator.  Each user instantiates their own submission site, typically by
using a virtual machine appliance, that connects to the rest of the system
through the GroupVPN, enabling them to connect from anywhere without
configuration.  Archer allows users to easily add resources using similar
mechanisms.  In the case of decentralized VPNs, this would be difficult as the
...
the central component went offline.  Archer and Grid Appliance would not be
possible without the GroupVPN architecture described in this paper. 
